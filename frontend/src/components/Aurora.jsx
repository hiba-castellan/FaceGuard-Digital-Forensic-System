...
#define COLOR_RAMP(colors, factor, finalColor) {              \
  ...
  for (int i = 0; i < 2; i++) {                               \
     ...
     bool isIn = cc.position <= factor;                       \
     index = int(mix(float(index), float(i), float(isIn)));   \
  }                                                           \
...
export default function Aurora({ colorStops = ['#00d4ff', '#7c3aed', '#00d4ff'], amplitude = 1.0, blend = 0.5, speed = 1.0 }) {
  ...
  useEffect(() => {
    ...
    const toArr = hex => { const c = new Color(hex); return [c.r, c.g, c.b]; };
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorStops: { value: colorStops.map(toArr) },
    ...
    const update = t => {
      ...
      program.uniforms.uColorStops.value = (p.colorStops ?? colorStops).map(toArr);
